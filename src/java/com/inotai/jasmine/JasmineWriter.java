...
import static com.inotai.jasmine.value.StringType.DOUBLE_QUOTED;
import static com.inotai.jasmine.value.ValueType.BOOLEAN;
...
public class JasmineWriter {
    ...
    public void write(Value value, StringBuilder builder) {
        ...
        switch (value.getType()) {
            ...
            case REGEXP:
                ...
                JasmineStringUtils.writeRegEx(reValue.getRegExp(), reValue.getRegExpOptions(), builder);
                this.formatter.afterValue(this.stack, builder);
                break;
    ...
    public void writeDictionary(DictionaryValue value, StringBuilder builder) {
        ...
        for (Iterator it = dictionaryValue.iterator(); it.hasNext(); ) {
            ...
            this.formatter.beforeDictionaryKey(this.stack, pair.getValue(), pair.getKey(), i, last, builder);
            JasmineStringUtils.writeStringUnquoted(pair.getKey(), builder);
            this.formatter.afterDictionaryKey(this.stack, pair.getValue(), pair.getKey(), i, last, builder);
            builder.append(':');
